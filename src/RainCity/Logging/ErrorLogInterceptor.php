...
{
    use SingletonTrait {
        SingletonTrait::__construct as trait__construct;
    }
    ...
    protected function __construct(array $args)
    {
        if (!empty($args)) {
            ...
            if (is_array($ignoreErrors)) {
                ...
                if (empty($this->ignoreErrors)) {
                    throw new \InvalidArgumentException('No valid error types/numbers provided');
                } else {
                    $this->trait__construct();
                }
            } else {
                throw new \InvalidArgumentException('Constructor argument is not an array of errors to ignore');
            }
        } else {
            throw new \InvalidArgumentException('No errors provided');
        }
    }
